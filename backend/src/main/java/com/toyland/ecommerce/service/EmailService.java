...
public class EmailService {
    ...
    public boolean sendPasswordResetEmail(
        ...
        System.out.println(
                "🔑 PASSWORD RESET EMAIL GENERATED FOR: " + toEmail
        );
        System.out.println(
        ...
        String activeApiKey =
                (brevoApiKey != null && !brevoApiKey.trim().isEmpty())
                        ? brevoApiKey.trim()
                        : (mailPassword != null
                        && (mailPassword.startsWith("xkeysib")
                        || mailPassword.startsWith("xsmtpsib"))
                        ? mailPassword.trim()
                        : "");
        ...
        if (fromEmail == null || fromEmail.trim().isEmpty()) {
            ...
            System.err.println(
                    "⚠️ EMAIL WARNING: MAIL_USERNAME environment variable is NOT set on Render."
            );

            logger.warn(
                    "⚠️ EMAIL WARNING: MAIL_USERNAME is not set."
            );
        ...
        CompletableFuture.runAsync(() -> {
            ...
            if (!activeApiKey.isEmpty()) {
                ...
                System.out.println(
                        "⚠️ Brevo HTTP API dispatch failed. Attempting SMTP relay fallback..."
                );
            }
            ...
            if (mailSender != null) {
                ...
                try {
                    ...
                    System.out.println(
                            "✅ Successfully dispatched SMTP password reset email to "
                                    + toEmail
                    );
                    ...
                    System.err.println(
                            "❌ SMTP ERROR: Failed to send password reset email to "
                                    + toEmail
                                    + ". Error: "
                                    + e.getMessage()
                    );
                    ...
                    logger.error(
                            "❌ SMTP ERROR: Failed to send password reset email to {}. Error: {}",
                            toEmail,
                            e.getMessage(),
                            e
                    );
                ...
                System.err.println(
                        "⚠️ SMTP WARNING: JavaMailSender is not initialized."
                );
            }
        });
